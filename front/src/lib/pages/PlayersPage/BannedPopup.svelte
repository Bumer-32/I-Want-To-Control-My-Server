...
<div class="absolute top-0 flex h-full w-screen items-center justify-center hidden" bind:this={container}>
    ...
    <div class="absolute z-[100] items-center rounded-[10px] bg-[var(--players-background-color)] pl-px">
        <div class="p-[10px] w-[300px] flex flex-col items-center gap-[5px]">
            <h1>Banned</h1>
            ...
            {#key key}
                <ul class="w-full max-h-[500px] overflow-auto py-[5px]" class:hidden={addresses}>
                    <li class="flex justify-center items-center [&:not(:only-child)]:hidden">No banned players</li>
                    {#await fetch(Constants.BAN_LIST_PLAYER_URL).then(r => r.json())}}
                    {:then list}
                        {#each list as target}
                            <li class="mt-[5px] w-full flex justify-around">
                ...
                <ul class="w-full max-h-[500px] overflow-auto py-[5px]" class:hidden={!addresses}>
                    <li class="flex justify-center items-center [&:not(:only-child)]:hidden">No banned addresses</li>
