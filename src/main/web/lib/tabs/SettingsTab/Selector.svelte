...
<div class="selector absolute top-[25px] flex items-center">
    <!-- ? Add underline -->
    <span class="bg-[var(--main-text-color)] absolute bottom-[-2px] h-[2px] rounded-[2px] pointer-events-none left-0 w-[126px]" style="transition: left 0.3s ease, width 0.3s ease;" bind:this={selectorUnderline}></span>

    <div class="flex items-center" bind:this={selectorContainer}>
        <!-- * Content will be auto generated by js -->
    </div>

    <div class="flex absolute right-[-100px] gap-[10px] justify-center items-center">
        <span class="material-symbols-rounded cursor-pointer w-[24px] text-[24px] text-center" style="transition: font-size 0.05s ease;" title="Update" bind:this={updateButton}>update</span>
        <span class="material-symbols-rounded cursor-pointer w-[24px] text-[24px] text-center" style="transition: font-size 0.05s ease;" title="File view" bind:this={fileViewButton}>file_open</span>
        <span class="material-symbols-rounded cursor-pointer w-[24px] text-[24px] text-center" style="transition: font-size 0.05s ease;" title="Save" bind:this={saveButton}>save</span>
    </div>
</div>
...
<style lang="scss">
  ...
  .selector {
    .material-symbols-rounded:active { // idk why, but tailwind with active:text-[19px] doesn't work
      font-size: 19px;
    }
